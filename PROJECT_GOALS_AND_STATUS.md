...
2. 2026-09-08 全量審查（原始證據已併入 [文件總覽](docs/DOCUMENTATION.md)，19 項）→ Phase 1+2 remediation（`7c5d4d5`，獨立驗收通過）＋ 最終標靶修復 F15/F19/NEW-R1（`0804814`）＋ CI hermetic 與 LLM token 成本歸戶（`1e79e19`/`617ba3b`）。
...
6. 2026-09-11 paper-readiness 審查（原始證據已併入 [文件總覽](docs/DOCUMENTATION.md)，16 項 R01–R16，結論「尚不可進入無人值守 Paper」）→ Plan-A（`bc06dac`：R03/R04/R05/R06/R07/R09/R14 broker/execution 安全）＋ Plan-B（`050b820`：R01/R02/R08/R10–R13/R15/R16 runtime/scheduler/state/reporting）＋ 最終 blockers（`00496c8`：R05 clock TOCTOU、R01 設定先於 recovery、R02 no-POST 狀態語意）＋ R01 WebUI runtime 完整性（`486aa74`）。
...
- [x] 2026-09-25：Provider failover 路由策略更新為共享預算下的交替重試：`Primary → Fallback → Primary → Fallback`（總請求數仍為 `1 + llm_max_retries`；永久錯誤仍立即停止，schema 驗證錯誤仍不觸發路由切換）；`llm_provider_failover` 審計事件記錄每次實際的 from/to 路由變化。專項回退與 schema 測試通過。
- [x] 2026-09-07~08：DVI-1 決策完整性與 `last_equity` daily-loss 接線、誠實績效呈現；DMC-1 辯論最小一致性修復；CI 驗收測試 hermetic 化；無歸戶 LLM token 成本修正。2026-09-08 全量審查（19 項，原始證據已併入 [文件總覽](docs/DOCUMENTATION.md)）→ **Phase 1+2 remediation 獨立驗收通過**（`7c5d4d5`）＋ 最終標靶修復 F15（nested LangChain usage 少計）/F19（resume 重入 screening 繞過預算）/NEW-R1（protected SHORT close 數量符號）（`0804814`）。
...
- [x] 2026-09-11：無人值守安全修復 F01–F10（`3c8d5b9`）＋ F02/F04/F06（`9dbc338`）＋ F02 殘餘 race（`a8c5371`：dispatch-time generation 捕捉、stale screening 不能停新 run），新增 56 項回歸（`tests/test_unattended_safety_regressions.py`）。同日 paper-readiness 獨立審查（基準 `a8c5371`）確認 16 項問題 R01–R16（11 P1、5 P2），結論「尚不可進入無人值守 Paper Trading」（原始證據已併入 [文件總覽](docs/DOCUMENTATION.md)）。
...
- [x] 2026-09-15：**schema 驗證穿透修復＋30 日觀察正式啟動**。觸發事故：第二個窗口（`lr-20260914T115852Z-dde155`，Agnes 主力）第一夜在 SWKS 上 agnes-3.0-flash 回 `{'recommendation': 'NEUTRAL'}` 缺 ResearchPlan 三個必填欄位——*成功回覆上的 schema 不合規*被 retry owner catch-all 誤判 permanent 並包成 ProviderFailure，單次不合規即整場硬停（該窗口誠實結案：9 標的 DONE、1 FAILED、10 PENDING、零 broker 曝險）。修復：`retry.py` 兩個 controller（`_RetryController`／`_FailoverRetryController`）將 pydantic `ValidationError` 與 `OutputParserException` 原樣穿透，交回節點邊界既有語義（Research Manager free-text fallback、Risk 嚴格 INVALID/NO_TRADE、screening `SCREENING_INVALID_OUTPUT`）；傳輸失敗重試預算與 401 即停政策不變；實作的本就是把模組文件既有合約寫進程式碼。新增 `tests/test_schema_validation_passthrough.py` 8 項（以真實 `ResearchPlan` 復現事故形狀＋failover 不誤觸發＋非回歸锚點）。另修兩項環境泄漏測試（`patch.dict` 只覆蓋不移除環境鍵：`_hermetic_env` 以清空字串取代 pop；phase_c 補空 `SCREENING_OPENAI_API_KEY`）——此缺陷因開發 `.env` 新增 role-specific key 而暴露，CI（無 .env）原本偵測不到。全 suite `1203 passed, 0 failed, 257 subtests`、compileall 綠。第三個窗口 `lr-20260915T035942Z-41038e` 同日經預設互動授權啟動：22 sessions、每日 11:00 ET、$1,000 notional、Agnes 主力＋本機 `gemini-3.8-flash`（`127.0.0.1:3000/v1`）failover、`allow_shorts=true`；第一窗口 `lr-20260913T050918Z-bad3cb` 因模型配置換代退役（0 session 執行、INTERRUPTED 收尾、無在途單）。
