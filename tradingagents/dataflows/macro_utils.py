...
def get_fred_data(series_id: str, start_date: str, end_date: str) -> Dict:
    ...
    try:
        # Finite connect/read timeout: a stalled FRED request must not hang
        # the synchronous analysis loop indefinitely.
        response = requests.get(url, params=params, timeout=(3.05, 10.0))
...
def get_treasury_yield_curve(curr_date: str) -> str:
    ...
    if yield_data:
        ...
        if two_year and ten_year:
            # F16: FRED yields are percentage points; 4.00 - 3.00 = 1.00
            # point = 100 basis points. Convert before display and
            # classification so the 50-bps threshold keeps its meaning.
            spread_bps = (ten_year["yield"] - two_year["yield"]) * 100.0
            result += f"- **2Y-10Y Spread**: {spread_bps:.2f} basis points\n"
