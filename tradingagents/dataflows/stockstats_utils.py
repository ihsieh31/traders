    ...
    def get_stock_stats(
        ...
        try:
            ...
            if "Date" not in data.columns:
                raise ValueError("Indicator data is missing Date/timestamp")
            for column in ("open", "high", "low", "close", "volume"):
                if column not in data.columns and column.title() in data.columns:
                    data[column] = data[column.title()]
            data["Date"] = pd.to_datetime(data["Date"])
            cutoff = curr_date_dt.strftime("%Y-%m-%d")
            data = data[data["Date"].dt.strftime("%Y-%m-%d") <= cutoff]
            data = data.sort_values("Date").reset_index(drop=True)
