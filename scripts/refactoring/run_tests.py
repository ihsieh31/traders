...
def main():
    ...
    env = {
        ...
        'TRADINGAGENTS_LONG_RUN_DIR': str(dirs['state'] / 'long_run'),
        'TRADINGAGENTS_EXECUTION_DB': str(dirs['state'] / 'execution.sqlite3'),
        'TRADINGAGENTS_EXECUTION_LOCK_DIR': str(dirs['locks']),
        'TRADINGAGENTS_RESULTS_DIR': str(dirs['results']),
        'TRADINGAGENTS_CACHE_DIR': str(dirs['cache']),
        'TRADINGAGENTS_MEMORY_LOG_PATH': str(dirs['state'] / 'memory.md'),
        'TRADINGAGENTS_AGENT_MEMORY_DIR': str(dirs['state'] / 'agent_memory'),
